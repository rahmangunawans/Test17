...
- July 31, 2025. Implemented fallback scraping system: created basic episode scraper that bypasses M3U8 extraction to avoid network issues, added "Basic Mode (Fast)" button for reliable episode extraction without streaming URLs, implemented automatic fallback when full scraping fails, successfully tested basic scraper extracting 10+ episodes reliably, provides comprehensive error categorization (SSL, DNS, timeout, connection errors) with specific user guidance
