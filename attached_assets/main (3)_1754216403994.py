...
class EnhancedIQiyiExtractor:
    ...
    def extract_dash_query(self, player_data: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            if urls:
                dash_query = urls[0]
                logging.info(f"✅ DASH query extracted: {dash_query}")
                return dash_query
            else:
    ...
    def get_m3u8_from_dash(self, dash_query: str) -> Optional[str]:
        ...
        try:
            ...
            video_data = data.get('data', {}).get('program',
                                                  {}).get('video', [])
            ...
            for video_item in video_data:
                if 'm3u8' in video_item and video_item['m3u8']:
                    ...
                    logging.info(
                        f"✅ M3U8 content found: {len(m3u8_content)} characters"
                    )
                    return m3u8_content
...
def home():
    ...
    return jsonify({
        'message': 'IQiyi M3U8 Extractor API',
        'version': '1.0',
        'endpoints': {
            'extract': '/api/extract - POST with url parameter',
...
def health_check():
    ...
    return jsonify({
        'status': 'healthy',
        'message': 'IQiyi Extractor API is running'
    })


@app.route('/api/extract', methods=['POST'])
def extract_m3u8():
    """Extract M3U8 from iQiyi URL"""
    try:
        ...
        if not data or 'url' not in data:
            return jsonify({
                ...
                'usage':
                'POST with JSON: {"url": "https://www.iqiyi.com/..."}'
            }), 400

        url = data['url']

        # Validate URL - support both iqiyi.com and iq.com
        if not (url.startswith('https://www.iqiyi.com/')
                or url.startswith('https://www.iq.com/')):
            return jsonify({
                'success':
                False,
                'error':
                'Invalid URL. Must be an iQiyi URL starting with https://www.iqiyi.com/ or https://www.iq.com/'
            }), 400
        ...
        logging.error(f"❌ Error in extract_m3u8 endpoint: {e}")
        return jsonify({
            'success': False,
...
def extract_m3u8_get():
    """Extract M3U8 from iQiyi URL via GET parameter"""
    try:
        ...
        if not url:
            return jsonify({
                ...
                'URL parameter is required',
                'usage':
                'GET /api/extract?url=https://www.iqiyi.com/...'
            }), 400
        ...
        logging.error(f"❌ Error in extract_m3u8_get endpoint: {e}")
        return jsonify({
            'success': False,
            'error': 'Internal server error',
            'details': str(e)
